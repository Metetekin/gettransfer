...
class Car extends Bookable
{
            ...
            if (!empty($this->extra_price)) {
                foreach ($this->extra_price as $k => $type) {
                    if (isset($extra_price_input[$k]) and !empty($extra_price_input[$k]['enable'])) {
                        ...
                        switch ($type['type']) {
                            case "one_time":
                                $type_total = $type['price'] * 1;
                                break;
                            case "per_day":
                                $type_total = $type['price'] * $duration_in_day * 1;
                                break;
                        }
    ...
    public function addToCartValidate(Request $request)
    {
        ...
        if(strtotime($start_date) < strtotime(datetime('Y-m-d 00:00:00')) or strtotime($start_date) > strtotime($end_date))
        {
            return $this->sendError(__("Your selected dates are not valid"));
        }
